<?php

namespace Tests\Feature;

use App\Models\ClassRoom;
use App\Models\Student;
use App\Models\Tutor;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Http\UploadedFile;
use Illuminate\Support\Facades\Storage;
use Spatie\Permission\Models\Role;
use Tests\TestCase;

class TraceScratchTest extends TestCase
{
    use RefreshDatabase;

    public function test_trace(): void
    {
        Storage::fake('public');
        Role::firstOrCreate(['name' => 'super_admin']);
        $user = User::create([
            'full_name' => 'Tester', 'email' => 't@e.com', 'username' => 't',
            'password' => bcrypt('password'), 'role' => 'super_admin', 'status' => 'active',
        ]);
        $user->assignRole('super_admin');

        $tutor = Tutor::create(['name' => 'Kak T', 'status' => 'active']);
        ClassRoom::create([
            'class_name' => 'Kelas Uji', 'class_category' => 'drawing', 'tutor_id' => $tutor->id,
            'capacity' => 5, 'schedule_date' => now()->toDateString(), 'schedule_time' => '09:00',
            'class_fee' => 100000,
        ]);
        $student = Student::create([
            'name' => 'Murid Uji', 'date_of_birth' => '2018-01-01', 'parent_name' => 'Wali',
            'phone_number' => '0812', 'class_type' => 'drawing', 'status' => 'active',
            'join_date' => now()->toDateString(),
        ]);

        try {
            $this->actingAs($user)->post('/reports', [
                'student_id' => $student->id,
                'period_start' => '2026-06-01',
                'period_end' => '2026-06-30',
                'activity_notes' => 'Perkembangan bagus',
                'achievement_score' => 90,
                'photo' => UploadedFile::fake()->image('pas.jpg', 400, 600),
            ]);
        } catch (\Throwable $e) {
            file_put_contents(
                'C:/Users/LENOVO/AppData/Local/Temp/claude/c--laragon-www-tarakan-art-class/2377a442-84e8-4820-a565-f1bdccf76a93/scratchpad/trace.txt',
                get_class($e)."\n".$e->getMessage()."\n".$e->getTraceAsString()
            );
        }

        $this->assertTrue(true);
    }
}
